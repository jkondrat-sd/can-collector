...
/*
void CStorage::log(uint16_t pid, int32_t values[], uint8_t count, const char* telemetryName) {
    char buf[256];
    byte n = snprintf(buf, sizeof(buf), "%s%X%c%d", telemetryName, pid, m_delimiter, values[0]);
    for (byte m = 1; m < count; m++) {
        n += snprintf(buf + n, sizeof(buf) - n, ";%d", values[m]);
    }
    dispatch(buf, n);
...
void CStorage::log(uint16_t pid, float values[], uint8_t count, const char* fmt)
{
    ...
    for (byte m = 0; m < count && (p - buf) < (int)sizeof(buf) - 3; m++) {
        if (m > 0) *(p++) = ';';
...
/*
void CStorage::log(uint16_t pid, float values[], uint8_t count, const char* fmt, const char* telemetryName)
{
    
    // char buf[256];
    char buf[512];
    char *p = buf + snprintf(buf, sizeof(buf), "%s%X%c", telemetryName, pid, m_delimiter);
    for (byte m = 0; m < count && (p - buf) < (int)sizeof(buf) - 3; m++) {
        if (m > 0) * (p++) = ';';
        int l = snprintf(p, sizeof(buf) - (p - buf), fmt, values[m]);
        char *q = strchr(p, '.');
